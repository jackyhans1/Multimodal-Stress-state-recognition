...
class Simple2DCNN(nn.Module):
    def __init__(self, out_dim=128, in_channels=3):
        super().__init__()
...
class Simple3DCNN(nn.Module):
    def __init__(self, out_dim=128, in_channels=3):
        super().__init__()
        self.features = nn.Sequential(
            nn.Conv3d(in_channels, 16, kernel_size=(3,3,3), padding=1), nn.BatchNorm3d(16), nn.ReLU(),
            nn.MaxPool3d((1,2,2)),  # T x H/2 x W/2
            nn.Conv3d(16, 32, (3,3,3), padding=1), nn.BatchNorm3d(32), nn.ReLU(),
            nn.MaxPool3d((2,2,2)),  # T/2 x H/4 x W/4
            nn.Conv3d(32, 64, (3,3,3), padding=1), nn.BatchNorm3d(64), nn.ReLU(),
            nn.MaxPool3d((2,2,2)),  # T/4 x H/8 x W/8
